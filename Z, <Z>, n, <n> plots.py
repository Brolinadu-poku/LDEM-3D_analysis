...
def create_plots(results, output_directory):
    ...
    for i, (phi, phi_results) in enumerate(results.items()):
        ...
        for j, result in enumerate(phi_results):
            ...
            axes[0].set_xlabel(r'$\dot{\gamma}$')
            axes[0].set_ylabel('Viscosity')
            axes[0].set_title('Viscosity vs Shear Rate')
            axes[0].legend()
            ...
            axes[1].plot(result['shear_rate'], result['Z_t'], 
                         alpha=0.7, linewidth=1, label=f"Run {j+1}", color=color)
            axes[1].set_xlabel(r'$\dot{\gamma}$')
            axes[1].set_ylabel('Z(t)')
            axes[1].set_title('Instantaneous Coordination Number vs Shear Rate')
            axes[1].legend()
            ...
            axes[2].plot(result['shear_rate'], result['n_t'], 
                         alpha=0.7, linewidth=1, label=f"Run {j+1}", color=color)
            axes[2].set_xlabel(r'$\dot{\gamma}$')
            axes[2].set_ylabel('n(t)')
            axes[2].set_title('Fraction with Contacts vs Shear Rate')
            axes[2].legend()
            ...
            axes[3].plot(result['shear_rate'], result['Z_moving_avg'], 
                         alpha=0.7, linewidth=1, label=f"Run {j+1}", color=color)
            axes[3].set_xlabel(r'$\dot{\gamma}$')
            axes[3].set_ylabel('<Z>(t)')
            axes[3].set_title('Moving Averaged Coordination Number vs Shear Rate')
            axes[3].legend()
            ...
            axes[4].plot(result['shear_rate'], result['n_moving_avg'], 
                         alpha=0.7, linewidth=1, label=f"Run {j+1}", color=color)
            axes[4].set_xlabel(r'$\dot{\gamma}$')
            axes[4].set_ylabel('<n>(t)')
            axes[4].set_title('Moving Averaged Fraction vs Shear Rate')
            axes[4].legend()
    ...
    for i, (phi, phi_results) in enumerate(results.items()):
        ...
        for result in phi_results:
            all_shear_rates.append(result['shear_rate'])
        ...
        if len(phi_results) > 1:
            shear_rate = np.mean(all_shear_rates, axis=0)
    ...
    axes[0].set_xlabel(r'$\dot{\gamma}$')
    axes[0].set_ylabel('Viscosity')
    axes[0].set_title('Viscosity vs Shear Rate - All φ')
    axes[0].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    axes[0].grid(True, alpha=0.3)
    
    axes[1].set_xlabel(r'$\dot{\gamma}$')
    axes[1].set_ylabel('Z(t)')
    axes[1].set_title('Instantaneous Coordination Number vs Shear Rate - All φ')
    axes[1].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    axes[1].grid(True, alpha=0.3)
    
    axes[2].set_xlabel(r'$\dot{\gamma}$')
    axes[2].set_ylabel('n(t)')
    axes[2].set_title('Fraction with Contacts vs Shear Rate - All φ')
    axes[2].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    axes[2].grid(True, alpha=0.3)
    
    axes[3].set_xlabel(r'$\dot{\gamma}$')
    axes[3].set_ylabel('<Z>(t)')
    axes[3].set_title('Moving Averaged Coordination Number vs Shear Rate - All φ')
    axes[3].legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    axes[3].grid(True, alpha=0.3)
    
    axes[4].set_xlabel(r'$\dot{\gamma}$')
    axes[4].set_ylabel('<n>(t)')
    axes[4].set_title('Moving Averaged Fraction vs Shear Rate - All φ')
...
if __name__ == "__main__":
    ...
    output_directory = Path("/Volumes/T7 Shield/3D Analysis 10r/plots")
