...
def create_photorealistic_particle_plot(particles_frame, coordination_numbers, metadata, timestep, 
    ...
    # Decide rendering method based on particle count
    use_3d_spheres = len(particles_frame) <= max_particles_for_spheres
    
    print(f"Rendering {len(particles_frame)} particles using {'photorealistic 3D spheres' if use_3d_spheres else '2D scatter'}...")
    
    if use_3d_spheres:
        # Render particles as photorealistic 3D spheres
        for i, (pos, radius, coord_num) in enumerate(zip(positions, radii, coordination_numbers)):
            ...
            # Create realistic material appearance with lighting
            ...
            # Add subtle edge highlighting for more realistic look
    ...
    rendering_type = "Photorealistic 3D Spheres" if use_3d_spheres else "Enhanced 2D Scatter"
...
def process_3d_visualization(phi_val, timestep_range=None, create_animation_flag=False,
                            sphere_resolution=20, max_particles_for_spheres=300):
            ...
            for m, run in enumerate(numRun):
                ...
                for par_file in par_files:
                    ...
                    for t in timesteps_to_plot:
                        ...
                        save_path = output_path / f"photorealistic_3d_phi_{phi_str}_timestep_{t:04d}_{base_name}.png"
...
def create_photorealistic_plots_all_phi(timestep_range=None, sphere_resolution=20, max_particles_for_spheres=300):
    """Create photorealistic 3D plots for all phi values."""
    print("Creating photorealistic 3D particle plots...")
...
def create_single_phi_photorealistic(phi_val, timesteps=None, sphere_resolution=20, max_particles_for_spheres=300):
    """Create photorealistic visualization for a single phi value."""
    print(f"Creating photorealistic 3D visualization for phi = {phi_val}")
...
if __name__ == "__main__":
    print("="*70)
    print("PHOTOREALISTIC 3D PARTICLE VISUALIZATION")
    print("="*70)
    
    # Create photorealistic visualization for first phi value
    print("Creating photorealistic test visualization for phi = 0.52...")
    create_single_phi_photorealistic(0.52, timesteps=[0, 10, 20], 
                                    sphere_resolution=25, max_particles_for_spheres=400)
    
    # Option 2: Create photorealistic plots for all phi values (uncomment to run)
    # create_photorealistic_plots_all_phi(timestep_range=(0, 5), sphere_resolution=20, max_particles_for_spheres=300)
    
    print(f"\nPhotorealistic 3D visualizations saved to: {output_path}")
    print("Files: photorealistic_3d_phi_X.XX_timestep_XXXX_filename.png")
    print("\nFeatures:")
    print("- Realistic 3D spheres with proper lighting and shading")
    print("- Transparent box container")
    print("- Material-like appearance")
    print("- Enhanced visual quality with anti-aliasing")
