...
def create_photorealistic_particle_plot(particles_frame, coordination_numbers, metadata, timestep, 
    ...
    """Create a photorealistic 3D plot of particles as VPython-style spherical balls with colored Zi legend."""
    ...
    light = LightSource(azdeg=225, altdeg=30)
    ...
    # Create a proper colorbar using the coordination numbers
    scalar_map = cm.ScalarMappable(norm=norm, cmap=cmap)
    scalar_map.set_array(coordination_numbers)
    cbar = plt.colorbar(scalar_map, ax=ax, shrink=0.6, aspect=25, pad=0.1)
...
def create_photorealistic_plots_all_phi(timestep_range=None, sphere_resolution=40):
    ...
    # Define timesteps from 0 to 2000 with step 50
    custom_timesteps = list(range(0, 2001, 50))  # 0, 50, 100, ..., 2000
    
    for phi_val in phi:
        print(f"\nProcessing phi = {phi_val}")
        process_3d_visualization(phi_val, timestep_range=(0, 2000), create_animation_flag=False,
                                sphere_resolution=sphere_resolution)
...
if __name__ == "__main__":
    ...
    print("Creating VPython-style 3D spheres for all phi values with custom timesteps...")
    create_photorealistic_plots_all_phi(timestep_range=(0, 2000), sphere_resolution=40)
