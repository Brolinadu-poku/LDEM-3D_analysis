...
for i, phii in enumerate(phi):
    ...
    for j, arj in enumerate(ar):
        for k, vrk in enumerate(vr):
            dataname = f"{topDir}/phi_{phii}/ar_{arj}/Vr_{vrk}/run_{numRun}"
            ...
            for par_file in par_files:
                ...
                plt.plot(timesteps[:min_length], total_frictional_contacts, label='$Z_t = \\text{frictional contacts per timestep}$', color='b')
                plt.xlabel('Time, secs')
                plt.ylabel('$Z_t = \\text{frictional contacts per timestep}$')
                plt.title(f'Total Frictional Contacts vs Time (phi={phii}, ar={arj}, vr={vrk})')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plt.show()

                print(f"Processed {par_file}. Adjacency matrices and total frictional contacts stored in memory.")